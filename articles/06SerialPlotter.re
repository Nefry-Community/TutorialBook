...
//}
シリアルモニタは、シリアル通信のデータをポート単位で待ち構えて、データが来るとデータの内容をテキストで表示します。
Nefry BTでプログラムの動作を確認したい時は、シリアル通信を使って何の処理をしているのかテキストで出力すると、プログラムの流れが把握しやすいでしょう。

//image[06SerialPlotter_03][シリアルプロッタの例][scale=0.8]{
//}
シリアルプロッタは、シリアル通信のデータをポート単位で待ち構えて、できる限り数字に置き換えて折れ線グラフで表示します。
センサの値をそのままグラフにすることができるので非常に便利なツールです。
...
繰り返し動作するloop部分では、 @<chap>{03DefaultLed}のLEDの制御に加えて、今回はSWを押した時に @<code>{Serial.println(10);} PCに対してシリアル通信で10という数字を送っています。
...
この時右下に並んだボタンの中央を選択して、ボーレートといわれるシリアルの通信速度を設定します。
Nefryと合わせる必要があり、Nefryでは「115200bps」という速度で通信しているのでそれに合わせると文字が見れると思います。
...
シリアルモニタと同様に右下のボタンを選択して、「115200bps」に合わせるとグラフが見れると思います。
