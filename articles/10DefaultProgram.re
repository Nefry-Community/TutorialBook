...
===[column] Nefryに何が書き込まれているのか確認する方法

@<chap>{11NefryWiFi}を参考にして設定画面を開くと、画像の青で囲われた部分にプログラム名が表示されます。

デフォルトプログラムの場合、「NefryBT Default Program」と表示されていますのでそれを見て確認してください。
//image[Program][プログラム名][scale=0.6]{
//}
...
このプログラムはNefry BTライブラリ1.2.1以降の場合、サンプルコードは@<tt>{NefryBTdefaultprogram}から確認することができます。
開き方は、この本の「はじめに」の「サンプルプログラムについて」の項目で説明しているのでそちらをご覧ください。
